...
JmeterPath=${WORKSPACE}/Jmeter_installation/apache-jmeter-5.0/bin
...
sudo truncate -s 0 ${WORKSPACE}/Automation/bin/jmeter.log
...
scriptToRun=${WORKSPACE}/Automation/tools/logAnalysis.sh
runMultiServer runRemoteScript 

#run logAnalysis.py on remote Pserver 
pythonScriptToRun=${WORKSPACE}/Automation/tools/logAnalysis.py
runMultiServer runRemotePythonScript

# run Sqls statistics from logs
scriptToRun=${WORKSPACE}/Automation/tools/SqlLogToJmeter.sh
runMultiServer runRemoteScript
...
cp ${WORKSPACE}/Automation/bin/jmeter.log ${suiteResultsFolder}/${testFolder} > /dev/null 2>&1
