...
def print_array_one_row(arr, arr_name):
    print()
    print("Printing:   ", arr_name)
    arr_len = len(arr)
    arr_index = 0
    for item in arr:
...
def print_array_seperate_rows(arr, arr_name):
    print()
    print("Printing:   ", arr_name)
    for item in arr:
        print(item)

def print_all_tests_indexes():
    print()
    print("Printing:   g_all_tests_indexes")
    test_index = 1
    for test in g_all_tests_indexes:
...
def filter_all_sql_count_table():
    ...
    try:
        ...
        header_index = 0
        full_name = os.path.basename(file_path)
        final_out_path = OUTPUTS_DIR + full_name
        with open(final_out_path, "w", encoding='utf-8', newline='') as out_file:
            for line in content:
                if 0 == header_index:
                    out_file.write(line)
                    header_index += 1
    ...
    # Update new filtered file path
    g_ordered_files[0][0] = final_out_path
    return Status.OK
        ...
        for file_path in g_ordered_files[test_index]:
            if is_file_qualified_for_processing(file_path, [ALL_AGGREGATE_REPORT_FILE_SUFFIX]) or \
                ...
                if Status.OK != status_result:
                    ...
                    return status_result
                else:  # Update new filtered file path
                    full_name = os.path.basename(file_path)
                    final_out_path = OUTPUTS_DIR + full_name
                    g_ordered_files[test_index][file_index] = final_out_path
...
def convert_text_file_to_sheet_with_images(text_file_path, file_iterator, workbook, delimiter):
    ...
    for row in text_content:
        ...
        if is_test_info:  # if row is test info
            temp_line.append('is_test_info')
            temp_line.append(row_index)
            temp_line.append(row)
            for item in row_items:
                worksheet.write(row_index, col_index, item)
                col_index += 1
        elif is_space:  # if row is space
            temp_line.append('is_space')
            temp_line.append(row_index)
            temp_line.append(row)
            for item in row_items:
                worksheet.write(row_index, col_index, item)
        ...
        row_index += 1
        temp_test_lines.append(temp_line)
    g_all_tests_indexes.append(temp_test_lines)
...
def run_script(suite_path):
    ...
    if Status.OK != status_result and Status.ERR_DIR_EXISTS != status_result:
        ...
        return status_result
    if Status.ERR_DIR_EXISTS == status_result:
        status_result = delete_all_content_in_directory(OUTPUTS_DIR)
        if Status.OK != status_result:
            print("deleting all files in directory failure")
            return status_result
...
def main():
    ...
    # suite_path = ".\\input_files\\VANILLA_SEKELETON_LogsMetrics_20211119_020001\\"
    # suite_path = ".\\input_files\\VANILLA_SEKELETON_LogsMetrics_20211120_020001\\"
